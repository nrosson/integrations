              ...
              {needsAction.map(conn => (
                <div key={conn.id} className="flex items-center justify-between py-2.5">
                  <div>
                    ...
                    <p className="text-xs text-gray-600">{conn.registration}</p>
                  </div>
            ...
            {recent.map(event => (
              <div key={event.id} className="flex items-start gap-3 py-2.5">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-gray-600 truncate">{event.target}</p>
                </div>
                <p className="text-xs text-gray-600 shrink-0">{new Date(event.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
              </div>
            ))}
          </div>
        </div>
